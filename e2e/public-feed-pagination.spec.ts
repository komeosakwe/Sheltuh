 ...
 * via page.route() — never a real backend or AWS. See tests/LiveEventFeed.test.tsx
...
/**
 * Simulates a DynamoDB-style query+filter: the cursor advances over *raw*
 * table order in fixed-size pages, and category filtering is applied only
 * *after* each raw page is fetched — so a page can legitimately come back
 * with zero matching items while `nextCursor` still points further in.
 */
...
test("loads more than 20 records across multiple 'Load more' clicks", async ({ page }) => {
  ...
  await page.route("**/api-mock/events*", (route) => mockEventsApi(route, all));
...
test("discards a stale response after the filter changes again before the first request resolves", async ({
  ...
  await page.route("**/api-mock/events*", async (route) => {
    const url = new URL(route.request().url());
